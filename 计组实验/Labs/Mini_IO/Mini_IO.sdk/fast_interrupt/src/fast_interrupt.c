...
int main()
{
	...
	Xil_Out32(XPAR_AXI_INTC_0_BASEADDR + XIN_IMR_OFFSET, 0x2); //  设置intr[1]为快速中断模式
	Xil_Out32(XPAR_AXI_INTC_0_BASEADDR + XIN_IVAR_OFFSET + 4*1, (int)btnHandler); // 注册中断向量
...
//	Xil_Out32(XPAR_AXI_INTC_0_BASEADDR + XIN_IAR_OFFSET, 0xffffffff); 快速中断模式自动清除中断状态
}
